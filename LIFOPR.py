...
while 1:
    ...
    index = queue[-1]
    for i in range(ncust):
        if arrival_times[i] < currtime+service_times[index]:
            queue.append(i)
            # print(queue)
            service_times[index] -= (arrival_times[i]-currtime)
            currtime=arrival_times[i]
            f1 = 1
            arrival_times[i] = 1e18
            break
...
total_times = [abs(round((finish_times[i]-arrival_times2[i]), 4))
               for i in range(ncust)]

print(total_times)

# Time spent@waiting before being served (time spent in the queue)
wait_times = [abs(round((total_times[i] - service_times2[i]), 4))
              for i in range(ncust)]
...
print('Output:', '\n',
      ...
      'Expected wait time in line (Wq):', str(
          data['wait_times'].mean()), '\n',
      'Expected time spent on the system (Ws):', str(
          data.total_times.mean()), '\n',
